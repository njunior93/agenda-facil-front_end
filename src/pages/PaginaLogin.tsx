...
const PaginaLogin = () => {
  ...
  const schema = yup.object({
    ...
    lembrar: yup
      .boolean()
      .default(() => !!localStorage.getItem("lembrado_email")),
  });
  ...
  const fetchResetarSenha = async () => {
    ...
    if (!email) {
      setAlerta(
        alertaMensagem(
          "Por favor, insira seu email para receber as instruções de reset de senha.",
          "warning",
          <ReportProblemIcon />,
        ),
      );
      ...
      <div className="w-full max-w-5xl min-h-[500px] bg-white shadow-lg rounded-2xl flex flex-col overflow-hidden md:!flex-row">
        ...
        <section className="w-full md:!flex-1 flex flex-col items-center justify-center p-6 md:!p-8">
            ...
            <form onSubmit={handleSubmit(fetchEntrar)}>
              ...
              <div className="flex flex-wrap items-center justify-between mt-4 mb-4">
                <FormControlLabel
                  label="Lembrar-me"
                  control={
                    <Checkbox checked={lembrar} {...register("lembrar")} />
                  }
                />
                <Link onClick={fetchResetarSenha} href="#" underline="hover">
                  Esqueceu sua senha?
                </Link>
              </div>
          ...
          <footer className="mt-8 text-sm text-gray-600">
            Novo aqui?{" "}
            <Link
              href={loading ? undefined : "/criar-usuario"}
              underline="hover"
            >
              Crie sua conta{" "}
            </Link>
