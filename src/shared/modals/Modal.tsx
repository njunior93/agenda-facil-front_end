...
const ModalComponent = () => {
  ...
  } = useForm<IClienteInput>({ resolver: yupResolver(schemaCliente) } as any);
  const {
    register: registerAgendamento,
    handleSubmit: handleSubmitAgendamento,
    reset: resetAgendamento,
    control: controlAgendamento,
    watch: watchAgendamento,
    formState: { errors: errorsAgendamento },
  } = useForm<IAgendamentoInput>({
  ...
  return (
    <>
      <Modal
        open={abrirModal}
        onClose={(_event: object, reason) =>
          reason != "backdropClick" && setAbrirModal(false)
        }
      >
        <Box sx={style}>
          <Typography
          ...
          {tituloModal === "Editar Cliente" && (
            <form
              onSubmit={handleSubmitCliente(fetchEditarCliente)}
            ...
            >
              ...
              {/* Área de botões responsiva: Empilha no celular, alinha na horizontal no desktop */}
                ...
                <Button
                  ...
                  onClick={() => {
                    if (clienteLocalizado)
                      fetchExcluirCliente(clienteLocalizado.id);
                  }}
                  sx={{
                    backgroundColor: "rgb(241, 40, 26)",
                    color: "#fff",
                    fontWeight: "bold",
                    borderRadius: "20px",
                    border: "2px solid #ffffffff",
                    paddingX: 3,
                    "&:hover": { backgroundColor: "rgb(255, 0, 0)" },
                    "&.Mui-disabled": {
          ...
          {tituloModal === "Cadastrar Cliente" && (
            <form
              onSubmit={handleSubmitCliente(fetchCriarCliente)}
              className="flex flex-col gap-4"
            >
              <TextField
                label="Nome"
          ...
          {tituloModal === "Cadastrar Agendamento" && (
            <form
              className="flex flex-col gap-4"
              onSubmit={handleSubmitAgendamento(fetchCriarAgendamento)}
            >
              <Controller
                name="tipoServico"
                control={controlAgendamento}
                render={({ field }) => (
                  <TextField
                    ...
                    {...field}
                    sx={{ backgroundColor: "#E2E0E0", borderRadius: 1 }}
            ...
            <div className="flex flex-col gap-4">
              ...
              >
                <Typography variant="body1" className="mb-1">
                  ...
                  {agendamentoLocalizado.cliente?.nome}
                </Typography>
                <Typography variant="body2" className="mb-1">
                  <strong>Serviço:</strong> {agendamentoLocalizado.servico}
                </Typography>
                <Typography variant="body2">
                  <strong>Status:</strong>{" "}
                  {agendamentoLocalizado.status === "a"
                    ? "Agendado"
                    : agendamentoLocalizado.status === "f"
                      ? "Finalizado"
                      : "Cancelado"}
                </Typography>
              </Box>
              ...
              <div className="flex flex-col sm:!flex-row gap-2 justify-end">
                {agendamentoLocalizado.status === "a" && (
                  <>
                    <Button
                      onClick={() => {
                        fetchEditarAgendamento({ status: "f" });
                      }}
                      sx={{
                        backgroundColor: "rgb(53, 163, 20)",
                        color: "#fff",
                    ...
                    <Button
                      onClick={() => {
                        fetchEditarAgendamento({ status: "c" });
                      }}
                      sx={{
                        backgroundColor: "#f1941aff",
                        color: "#fff",
                ...
                {agendamentoLocalizado.status === "c" && (
                  <Button
                    disabled={loading}
                    onClick={() => {
                      fetchExcluirAgendamento(agendamentoLocalizado.id);
                    }}
                    sx={{
                      backgroundColor: "#f1941aff",
                      color: "#fff",
                ...
                <Button
                  ...
                  onClick={() => sairModal()}
                  sx={{
                    backgroundColor: "rgb(87, 84, 82)",
                    color: "#fff",
                    fontWeight: "bold",
                    borderRadius: "20px",
                    border: "2px solid #ffffffff",
                    paddingX: 3,
                    "&:hover": { backgroundColor: "rgb(27, 27, 27)" },
                    "&.Mui-disabled": {
